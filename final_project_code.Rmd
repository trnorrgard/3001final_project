...
boston_plot <- boston[c('price', 'review_scores_rating')]
boston_plot <- boston_plot[complete.cases(boston_plot),]

plot<- ggplot(boston_plot, aes(x=price, y=review_scores_rating))+
  geom_point()
```
...
## i re-did this  with 6 clusters since i made 6 neighborhood groups, idk which one is better
## yall can switch from 4 to 6 and see which one works better bc idk

We ran the clustering algorithm with 6 clusters
```{r, warning=FALSE}
set.seed(123)
kmeans_obj_boston = kmeans(clust_boston, centers = 6, 
                        algorithm = "Lloyd")
